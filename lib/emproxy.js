...
function readStdinOrArgs(callback) {
    ...
    if (process.argv.length >= 4) {
        if (process.argv[2] === "-config") {
            var cfgArg = process.argv[3];
            var cfg = cfgArg.substring(0, 1) == '@' ? 
                fs.readFileSync(cfgArg.substring(1)).toString() : cfgArg;
            callback(cfg);
            return;
        }
    }
    ...
    request.addListener("end", function() {
        ...
        requestDomain.run(function() {
            ...
            requestDomain.on('error', function(err) {
                ...
                RestResponse.errors.push({targetType:'CdmError',code:'integration.parseRequest',message:err.message,badRequest:postData,stack:err.stack})
